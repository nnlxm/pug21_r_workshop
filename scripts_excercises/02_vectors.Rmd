...
c(2, 3.36, 9, 5.0032)
# zeigt so viele Nachkommastellen, wie die längste eingegebene Dez.zahl
```
...
c(10, "Keks", TRUE) #geht nicht! dann wird alles als character gemacht
c(10, TRUE)         #jetzt wird alles als integer gemacht (steht über Zahl)
```
...
```{r}
combined_vector1 <- c(visual_acuracy, subjects)
combined_vector1 # Werte beider Variablen werden dann chronologisch hintereinander geschrieben
```
...
```{r}
c(1, 2, 5) + 1 # jedes Element des Vektors + 1
```
...
1.3) `c(1, 2, 5) * 2`
```{r}
c(1, 2, 5) * 2 # jedes Element des Vektors * 2
...
```{r}
c(2, 4, 6, 4, 9, 12) * c(1,2) # dh jedes 2. Element soll mit 2 multipliert werden
...
c(2, 4, 6, 4, 9, 12)-1:3 #die Konvention ist es wohl, es so zu schreiben
c(2, 4, 6, 4, 9, 12)-c(1:3, 1:3) #oder so
```

1.10) Run the following code cell. What happened here? Is that bad?

```{r}
c(2, 4, 6, 4, 9, 12) + c(1, 2, 3, 4) # die Fehlermeldung ist nicht schlimm, die Rechnung stimmt trotzdem noch - dann werden die hinteren Elemente des 2. Vektors nicht auch noch ein 2. Mal mit Elementen des 1. Vektors verrechnet
#meistens ist es aber nicht das, was wir wollen, also ist es gut, dass wir eine Fehlermeldung bekommen
```
...
c(hotdog_names[3], hotdog_names[5])
```
...
```{r}
hotdog_names[c(2:4)] - hotdog_names[c(1)]
```
...
```{r}
max(hotdog_2019)
hotdog_2019 == max(hotdog_2019) #das ist jetzt ein Index, um etwas in einem Vektor zu indentifizieren
index <- max(hotdog_2019)
index
...
index <- c(TRUE, TRUE, FALSE, TRUE) #macht wie ein Filter, dass das 3. Element nicht angezeigt wird
numbers[index]
```
...
# apply it to numbers
numbers[numbers <3] #gib mit den Numbers-Vektor, aber nur an denjenigen Positionen, in denen die Elemente <3 sind
```
...
numbers <- 1:10
numbers[numbers>6]
(1:10)[1:10>6] # geht auch, aber auf die ersten runden Klammern achten!
```
...
```{r}
hotdog_names[hotdog_2019 > hotdog_2018] #gib mir deren Namen, die... #= INDEX
hotdog_names[c(2,4)] #oder #=VECTOR
hotdog_names[which(hotdog_2019 > hotdog_2018)] # oder
```
...
```{r}
results2019_male <- hotdog_2019[hotdog_genders == "male"]
results2028_female<- hotdog_2018[hotdog_genders == "female"]
results2019_male 
results2028_female
```
...
female_results_2019
most_eaten_female <- female_names & female_results_2019 #??
...
```{r}
sd(c(1,2,NA,21,3))
#jetzt NA entfernen, sodass SD berechnet werden kann:
sd(c(1,2,NA,21,3), (na.rm = TRUE))
```
...
a_vector[a_vector == 12] # NA wird auch ausgeworfen, weil es da ja nicht sicher ist, ob das nicht auch ne 12 ist
```
...
a_vector ==NA
``` #Fehlermeldung! Das wäre grad doof, den Vektor zu überschreiben!
